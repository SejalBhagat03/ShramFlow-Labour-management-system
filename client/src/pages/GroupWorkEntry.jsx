...
const GroupWorkEntry = () => {
    ...
    const zeroRateLabours = useMemo(() => {
        return selectedLabours
            .map(id => ({ id, labour: labourers.find(l => l.id === id), rate: tempRates[id] }))
            .filter(item => item.rate === 0);
    }, [selectedLabours, labourers, tempRates]);
    ...
    const handleCreateGroup = () => {
        if (!date || !location || !taskType || selectedLabours.length === 0) {
    ...
    const handleSaveWork = async () => {
        ...
        if (totalArea <= 0 && !hasAnyMeters) {
            toast.error("Please enter the meters completed by the labourers.");
            return;
        }
        ...
        // Validate if any labour has 0 rate -> Warning instead of blocker
        const hasRateIssue = distribution.some(d => d.rateUsed === 0 && d.meters > 0);
        if (hasRateIssue) {
            const confirmed = window.confirm("One or more labourers have a Rate of 0. Their payment will be recorded as ₹0. Do you want to proceed?");
            if (!confirmed) return;
        }
                    ...
                    <>
                            ...
                            <Card className="shadow-card">
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Select value={projectId} onValueChange={setProjectId} disabled={!!activeProject}>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Select value={location} onValueChange={setLocation} disabled={!!activeProject}>
                                            <SelectTrigger className="h-12 text-lg">
                                            ...
                                            <SelectContent>
                                                ...
                                                {location && !areaKeys.includes(location) && (
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Select value={taskType} onValueChange={setTaskType}>
                                            ...
                                            <SelectContent>
                                                ...
                                                {taskType && !taskKeys.includes(taskType) && (
                                                    <SelectItem value={taskType}>{taskType}</SelectItem>
                                                )}
                                            </SelectContent>
                                        </Select>
                                    </div>
                        ...
                        <div className="flex justify-end">
                            <Button
                                ...
                                disabled={!date || !projectId || !location || !taskType || selectedLabours.length === 0}
                            >
                    ...
                    <>
                        ...
                        <Card className="shadow-card border-2 border-primary/20">
                            ...
                            <CardContent className="space-y-4 pt-6">
                                ...
                                {zeroRateLabours.length > 0 && (
                                    <Alert variant="destructive" className="bg-destructive/5 border-destructive/20">
                                        <AlertCircle className="h-4 w-4 text-destructive" />
                                        <AlertTitle className="text-destructive font-bold">Rate Attention Required</AlertTitle>
                                        <AlertDescription className="text-destructive/90">
                                            The following labourers have a rate of ₹0: 
                                            <span className="font-bold ml-1">
                                                {zeroRateLabours.map(item => item.labour?.name || 'Unknown').join(', ')}
                                            </span>. 
                                            Please set their rates below to calculate payment.
                                        </AlertDescription>
                                    </Alert>
                                )}
                                ...
                                <div className="space-y-3 pt-4">
                                    <Label className="text-sm font-bold flex items-center gap-2">
                                        ...
                                        Photo Evidence (Site Verification)
                                    </Label>
                                    <ImageUpload 
                                        value={evidenceUrl}
