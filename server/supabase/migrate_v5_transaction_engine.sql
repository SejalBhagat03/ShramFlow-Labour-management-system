...
BEGIN
    IF (TG_OP = 'UPDATE' AND OLD.status != 'approved' AND NEW.status = 'approved') OR
       (TG_OP = 'INSERT' AND NEW.status = 'approved') THEN
        ...
        INSERT INTO public.labour_ledger (labourer_id, organization_id, transaction_type, category, amount, description, reference_id, metadata)
        VALUES (
            NEW.labourer_id, 
            NEW.organization_id,
            'CREDIT', 
...
INSERT INTO public.labour_ledger (labourer_id, organization_id, transaction_type, category, amount, description, reference_id, metadata, created_at)
SELECT 
    labourer_id, 
    organization_id,
    'CREDIT', 
