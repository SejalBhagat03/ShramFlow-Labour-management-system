...
export const WorkClaimForm = ({ labourerId, userId }) => {
    ...
    useEffect(() => {
        if (todayClaim) {
            ...
            if (todayClaim.photo_url_after) setPhotoAfterPreview(todayClaim.photo_url_after);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            let photoUrlAfter = todayClaim?.photo_url_after || null;

            // Upload photos if new ones selected
            if (photoFile) {
                photoUrl = await uploadWorkEvidence(photoFile, userId, "before");
            }
            if (photoAfterFile) {
                photoUrlAfter = await uploadWorkEvidence(photoAfterFile, userId, "after");
            }
            ...
            <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                    <CardTitle className="text-lg flex items-center gap-2">
                        ...
                        {t("submitWorkClaim") || "Submit Work Claim"}
                    </CardTitle>
                    {hasExistingClaim && (
                        <Badge
                            variant="outline"
            ...
            <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                    <Label htmlFor="meters" className="flex items-center gap-2">
                        ...
                        {t("metersDigToday") || "Meters dug today"} *
                    </Label>
                    <Input
                        id="meters"
                        type="number"
                ...
                <div className="space-y-2">
                    ...
                    {location ? (
                        ...
                        >
                            ...
                            {t("captureLocation") || "Capture GPS Location"}
                        </Button>
                    )}
                </div>
