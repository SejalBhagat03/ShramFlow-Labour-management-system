    ...
    const createLabourer = useMutation({
        mutationFn: async (formData) => {
            ...
            if (formData.password && !emailToUse && formData.phone) {
                ...
                console.log("Auto-generated email for labourer:", emailToUse);
            }
            ...
            if (emailToUse && formData.password) {
                ...
                if (funcError) {
                    ...
                    // Try to parse the error body if it exists as a string
                    let errorMessage = funcError.message || "Unknown error";

                    // DEBUG: Log the full error object to understand its structure
                    console.log("FULL EDGE FUNCTION ERROR OBJ:", JSON.stringify(funcError, null, 2));
