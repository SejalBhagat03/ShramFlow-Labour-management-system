...
const GroupWorkEntry = () => {
    ...
    const handleCreateGroup = () => {
        ...
        setStep(2);
        toast.success("Group created! Now assign meters to each labour after work completion.");
    };
    ...
    const handleSaveWork = async () => {
        ...
        // Validate if any labour has 0 rate -> Warn user
        ...
        const bulkEntries = distribution.map(item => ({
            group_id: groupId,
        ...
        try {
            console.log("Saving Bulk Entries:", bulkEntries);
            ...
            console.error("Failed to save group work:", error);
                                    ...
                                    {selectedLabours.map(id => {
                                        ...
                                        return (
                                            <div key={id} className="grid grid-cols-2 items-center gap-4 p-4 rounded-xl bg-accent/30 border">
                                                <div>
                                                    <p className="font-bold text-lg">{labour?.name}</p>
                                                    <p className="text-sm text-muted-foreground">Actual meters completed (optional)</p>
                                                </div>
                                                <div className="flex gap-2 items-center">
                                                    <div className="relative flex-1">
                                                        <Input
                                                            type="number"
                                                            placeholder="Auto"
                                                            className="text-right h-12 text-xl font-bold pr-12"
                                                            value={labourMeters[id] || ''}
                                                            onChange={(e) => handleMeterChange(id, e.target.value)}
                                                        />
                                                        <span className="absolute right-4 top-1/2 -translate-y-1/2 font-bold text-muted-foreground">m</span>
                                                    </div>
                                                    <Button
                                                        variant="ghost"
                                                        size="icon"
                                        ...
                                        <p className="text-3xl font-black text-foreground">
                                            ₹{(() => {
                                                ...
                                                if (hasManualMeters) {
                                                    ...
                                                    selectedLabours.forEach(id => {
                                                        const l = labourers.find(x => x.id === id);
                                                        total += perLabour * (l?.rate_per_meter || 0);
                                                    });
                                                }
