...
const Login = () => {
    ...
    // 🔹 Reactive Navigation (Strict Requirement)
    useEffect(() => {
        // Avoid redirect while auth state is being initialized
        if (authLoading) return;

        console.log("Login: Nav Effect Triggered. Auth:", isAuthenticated, "Role:", user?.role);
        if (isAuthenticated) {
            console.log("Login: Authenticated, redirecting...");
    ...
    const handleSupervisorLogin = async (e) => {
        ...
        try {
            const { error } = await login(loginEmail, supervisorPassword);
    ...
    const handleSupervisorSignup = async (e) => {
        ...
        try {
            ...
            const { error } = await signup(emailToRegister, signupPassword, signupName, phoneToRegister);
    ...
    const handleLabourLogin = async (e) => {
        ...
        const { error } = await login(emailToLogin, labourPassword);
        ...
        if (error) {
            ...
            console.log("Labour Login success, waiting for redirect...");
        }
    };
